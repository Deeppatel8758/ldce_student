...
const StudentQueries = {
  ...
  userLogin: async function (data) {
    console.log("Enter in use login ##########################################################################");
    const user = await AllSchemaAccess.StudentCollegeDetails.findOne({
      where: {
  ...
  getStudentProfileDetails: async function (username) {
    ...
    const query =
      "SELECT * FROM studentcollegedetails INNER JOIN studentpersonaldetails ON studentcollegedetails.username=studentpersonaldetails.enrollment WHERE studentcollegedetails.username='" +
      username +
      "'";
