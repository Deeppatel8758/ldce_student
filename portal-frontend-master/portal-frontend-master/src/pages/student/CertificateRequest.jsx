...
export default function CertificateRequest() {
  ...
  const handleCertificateChange = async (e) => {
    ...
    const updateDataItem = (key, newValue,categorydatatype,categoryid,categoryname) => {
...
const inputIndex=[]
  ...
  let formdata = new FormData()
    ...
    const data = await axios.post(NetworkRoute.makeCertificateRequest,formdata, {
    headers: {
  ...
  return (
    ...
    <label className = "ml-2">Select Certificate</label>
      ...
      <select className="w-full mt-8 rounded-md border-none p-4"
         onChange={handleCertificateChange}
      >
    
        <option default value="">Select Certificate</option>
    {pickCertificate && pickCertificate.length > 0 && pickCertificate.map((certi, key) => {
      return(
        <option value = {certi.id} key={key}>{certi.certificatename?certi.certificatename:""}</option>
      )
    })}
      ...
      <form  onSubmit={MakeRequest} encType="multipart/form-data">
        ...
        <p className="text-xl font-bold text-center py-2">{allCertificateDetails?.certificatename}</p>
      <div className=" grid xl:grid-cols-3 md:grid-cols-2 sm:grid-cols-1  py-6">
         {allDocuments.length > 0 && allDocuments.map((doc,key) => {
            return (
            <div >
              <p className="text-md mt-4 font-bold">{doc?.categoryname}</p>  
              <div key={key}>
                {doc.categorydatatype === 'INTEGER' && (
                  <input
                    ...
                    onChange={(e) => updateDataItem(key,e.target.value?parseInt(e.target.value):undefined,doc.categorydatatype,doc.categoryid,doc.categoryname)}
                  />
                )}
                {doc.categorydatatype === 'DOUBLE' && (
                  <input
                    type="number"
                      ...
                      required
                  value={allInputValues[key]?.value}
                    onChange={(e) => updateDataItem(key, e.target.value?parseFloat(e.target.value):undefined,doc.categorydatatype,doc.categoryid,doc.categoryname)}
                  />
                )}
                {doc.categorydatatype === 'STRING' && (
                  <input
                      ...
                      required
                  value={allInputValues[key]?.value}
                    onChange={(e) => updateDataItem(key, e.target.value.length>0?e.target.value:undefined,doc.categorydatatype,doc.categoryid,doc.categoryname)}
                  />
                )}
                {doc.categorydatatype === 'FILE' && (
                  <input
                      type="file"
                      required
                    onChange={(e) => updateDataItem(key, e.target.files[0]?e.target.files[0]:undefined,doc.categorydatatype,doc.categoryid,doc.categoryname)}
                  />
                )}
              </div>    
            </div>
